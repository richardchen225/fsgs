...
def batchify_unproject_depth_map_to_point_map(
    ...
    intrinsics_cam, depth_map = intrinsics_cam.flatten(0, 1), depth_map.flatten(0, 1)
...
#     meshgrid_u = torch.arange(W, device=depth_map.device, dtype=torch.float32) / W 
#     meshgrid_v = torch.arange(H, device=depth_map.device, dtype=torch.float32) / H
    
#     # Expand 这里的逻辑没变
#     u = meshgrid_u[None, None, :].expand(batch_size * num_views, H, W)
#     v = meshgrid_v[None, :, None].expand(batch_size * num_views, H, W)
...
def batchify_sparse_unproject_depth_to_point(
    u_v_pixel: torch.Tensor, 
    sampled_depth: torch.Tensor, 
    extrinsics_cam: torch.Tensor, 
    intrinsics_cam: torch.Tensor
) -> torch.Tensor:
    """
    稀疏反投影：将指定的 K 个像素坐标及其深度值转换到世界坐标系。

    Args:
        u_v_pixel (torch.Tensor): 采样点的像素坐标 (B*V, K, 2)，最后一位是 [u, v]
        sampled_depth (torch.Tensor): 对应坐标的深度值 (B*V, K, 1)
        extrinsics_cam (torch.Tensor): 外参矩阵 (B, V, 3, 4)
        intrinsics_cam (torch.Tensor): 内参矩阵 (B, V, 3, 3)
        
    Returns:
        torch.Tensor: 3D 世界坐标 (B*V, K, 3)
    """
    # 1. 维度准备
    batch_size_total = u_v_pixel.shape[0] # 这里已经是 B*V
    K = u_v_pixel.shape[1] # 采样点数量
    
    # 展平内外参以匹配 (B*V)
    # 如果输入的 extrinsics 是 (B, V, 3, 4)，先 flatten 前两维
    if extrinsics_cam.dim() == 4:
        extrinsics_cam = extrinsics_cam.flatten(0, 1) # (S, 3, 4)
    if intrinsics_cam.dim() == 4:
        intrinsics_cam = intrinsics_cam.flatten(0, 1) # (S, 3, 3)
    ...
    u = u_v_pixel[..., 0] # (S, K)
    v = u_v_pixel[..., 1] # (S, K)
    z_cam = sampled_depth.squeeze(-1) # (S, K)

    # 4. 反投影到相机坐标系 (Camera Coordinates)
    # 使用公式: x = (u - cu) * z / fu
    x_cam = (u - cu[:, None]) * z_cam / fu[:, None]
    y_cam = (v - cv[:, None]) * z_cam / fv[:, None]
    
    # 组合成 (S, K, 3)
    cam_coords = torch.stack((x_cam, y_cam, z_cam), dim=-1)

    # 5. 变换到世界坐标系 (World Coordinates)
    # 构造齐次坐标: (S, K, 4)
    homo_pts = torch.cat(
        (cam_coords, torch.ones_like(cam_coords[..., :1])), 
        dim=-1
    ) # (S, K, 4)

    # 执行坐标变换: (S, 3, 4) * (S, 4, K) -> (S, 3, K)
    # 这里使用 bmm，需要将点云矩阵转置
    world_coords = torch.bmm(
        extrinsics_cam, 
        homo_pts.transpose(1, 2)
    ).transpose(1, 2) # (S, K, 3)

    return world_coords # 返回 (B*V, K, 3)
