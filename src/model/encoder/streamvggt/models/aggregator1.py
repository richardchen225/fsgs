...
from src.model.encoder.streamvggt.layers import PatchEmbed
from src.model.encoder.streamvggt.layers.block import Block
from src.model.encoder.streamvggt.layers.rope import RotaryPositionEmbedding2D, PositionGetter
from src.model.encoder.streamvggt.layers.vision_transformer import vit_small, vit_base, vit_large, vit_giant2
from src.model.encoder.streamvggt.layers.mamba import MambaModel
...
class Aggregator_self(nn.Module):
    """
    ...
    def forward(
        ...
        """
        # if self.mamba_flag == 0:
        #     for i in range(self.depth):
        #         if i in [4, 11, 17, 23]:
        #             self.mamba_blocks[i].start_new_sequence()
            # self.mamba_flag = 1
        ...
        for _ in range(self.aa_block_num):
            ...
            for i in range(len(frame_intermediates)):
                ...
                output_list.append(frame_intermediates[i])
    ...
    def _process_mamba(self, tokens, B, S, P, C, global_idx):
        """
        Process global attention blocks. We keep tokens in shape (B, S*P, C).
        """
       
        tokens = tokens.view(B,S,P,C)
        mamba_kwargs = dict(in_dpt_layer=[1])

        # mamba_out = torch.zeros_like(dpt_features)
        mamba_out = []
        
        for i in range(S):
            seq_out = self.mamba_blocks[global_idx].forward_single_frame(tokens[:,i,...], **mamba_kwargs)            
        ...
        intermediates = []
        intermediates.append(mamba_out)                     

        return mamba_out.view(B*S, P, C), global_idx, intermediates
