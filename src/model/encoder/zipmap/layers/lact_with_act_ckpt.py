...
class ComputeLactFastWeightGrads(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, W0, W1, W2, K, V, lr0, lr1, lr2, BatchSize):
        """
        Args:
        ...
        outs:
            ...
            DW0: [BatchSize, Hidden, D]
            DW1: [BatchSize, D, Hidden]
            DW2: [BatchSize, Hidden, D]
        """
        ...
        # note, K, V, lr0, lr1, lr2 are rearranged in the forward pass.
    ...
    def backward(ctx, grad_dw0, grad_dw1, grad_dw2):
        ...
        Outs:
            ...
            grad_W1: [B, D, Hidden]
            grad_W2: [B, Hidden, D]
            grad_K: [1, B * num_tokens, D]
            grad_V: [1, B * num_tokens, D]
            grad_lr0: [1, B * num_tokens]
            grad_lr1: [1, B * num_tokens]
            grad_lr2: [1, B * num_tokens]
        ...
        W0_repeated = W0.repeat(BatchSize, 1, 1)
        W1_repeated = W1.repeat(BatchSize, 1, 1)
        W2_repeated = W2.repeat(BatchSize, 1, 1)

        Y0 = torch.bmm(W0_repeated, K.transpose(1, 2))
        Y2 = torch.bmm(W2_repeated, K.transpose(1, 2))
        DHidden = torch.bmm(W1_repeated.transpose(1, 2), V.transpose(1, 2))
        ...
        grad_K = (
            grad_K
            + torch.bmm(grad_Y0.transpose(1, 2), W0_repeated)
            + torch.bmm(grad_Y2.transpose(1, 2), W2_repeated)
        )

        grad_V = torch.bmm(
            grad_DHidden.transpose(1, 2), W1_repeated.transpose(1, 2)
        ) + torch.bmm(Hidden_with_lr1.transpose(1, 2), grad_dw1.transpose(1, 2))

        grad_W1 = torch.bmm(V.transpose(1, 2), grad_DHidden.transpose(1, 2)).sum(
            dim=0, keepdim=True
        )
        # [B, Hidden, D] = [B, Hidden, num_tokens] @ [B, num_tokens, D]
        # grad_W0, grad_W2 = two_mm_same_inp_interface_v2(
        #     grad_Y0, grad_Y2, K, autotune=autotune
        # )
        grad_W0 = torch.bmm(grad_Y0, K).sum(dim=0, keepdim=True)
        grad_W2 = torch.bmm(grad_Y2, K).sum(dim=0, keepdim=True)

        ### reshape back
        grad_K = rearrange(grad_K, "(one b) n d -> one (b n) d", b=BatchSize)
        grad_V = rearrange(grad_V, "(one b) n d -> one (b n) d", b=BatchSize)
        grad_lr0 = rearrange(grad_lr0, "(one b) n -> one (b n)", b=BatchSize)
        grad_lr1 = rearrange(grad_lr1, "(one b) n -> one (b n)", b=BatchSize)
        grad_lr2 = rearrange(grad_lr2, "(one b) n -> one (b n)", b=BatchSize)
