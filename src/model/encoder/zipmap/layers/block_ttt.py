...
class Block(nn.Module):
    def __init__(
        ...
        if not self.ttt_mode:
            ...
            self.ttt_block = FastWeightGluMLPMultihead(
                dim,
                **ttt_params
            )
    ...
    def forward(self, x: Tensor, pos=None, info={}) -> Tensor:
        ...
        def ttt_residual_func(x: Tensor, info={}) -> Tensor:
            residual, fast_weight = self.ttt_block(self.norm1(x), info=info)
            return self.ls1(residual), fast_weight
        store_state = info.get("store_state", False)
        fast_weight = None
        if self.training and self.sample_drop_ratio > 0.1:
            # TODO: Need adaptation for TTT
            ...
            if self.ttt_mode:
                residual, fast_weight = ttt_residual_func(x, info=info)
