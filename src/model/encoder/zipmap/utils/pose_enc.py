...
def extri_intri_to_pose_encoding(
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        R_flat = R.reshape(R.shape[0], R.shape[1], 9)  # BxSx9
        # Note the order of h and w here
        H, W = image_size_hw
        fov_h = 2 * torch.atan((H / 2) / intrinsics[..., 1, 1])
        fov_w = 2 * torch.atan((W / 2) / intrinsics[..., 0, 0])
        pose_encoding = torch.cat([T, R_flat, fov_h[..., None], fov_w[..., None]], dim=-1).float()
...
def pose_encoding_to_extri_intri(
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        R = quat_to_mat(quat)
        extrinsics = torch.cat([R, T[..., None]], dim=-1) # w2c
        ...
        T = pose_encoding[..., :3]
        R_flat = pose_encoding[..., 3:12]
        fov_h = pose_encoding[..., 12]
        fov_w = pose_encoding[..., 13]

        R = R_flat.reshape(R_flat.shape[0], R_flat.shape[1], 3, 3)
        extrinsics = torch.cat([R, T[..., None]], dim=-1) # c2w (S, 3, 4)
