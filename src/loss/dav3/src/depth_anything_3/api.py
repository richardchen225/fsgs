...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    Usage:
        ...
        # Or create with a local checkpoint
        model = DepthAnything3(checkpoint_path="weights/pre_dav3.safetensors")
    ...
    def __init__(self, model_name: str = "da3-large", checkpoint_path: str | Path | None = None, **kwargs):
        """
        ...
        checkpoint_path: Path to the Depth Anything 3 safetensors checkpoint.
        **kwargs: Additional keyword arguments (currently unused).
        """
        super().__init__()
        self.model_name = model_name
        from safetensors.torch import load_file
        from .model.dualdpt import DualDPT
        head = DualDPT(dim_in = 2048,)
        from .model.dinov2.dinov2 import DinoV2
        net = DinoV2(name = 'vitl' , out_layers=[11, 15,  19,  23], alt_start= 8,qknorm_start= 8,rope_start= 8)
        from .model.cam_dec import CameraDec
        cam_dec = CameraDec(dim_in=2048)
        from .model.da3 import DepthAnything3Net
        self.model = DepthAnything3Net(net, head, cam_dec = cam_dec)
        if checkpoint_path is None:
            raise ValueError("checkpoint_path must be provided for DepthAnything3.")
        checkpoint_path = Path(checkpoint_path)
        if not checkpoint_path.exists():
            raise FileNotFoundError(f"Depth Anything 3 checkpoint not found: {checkpoint_path}")
    ...
    def inference(
        ...
        return prediction, pose_enc, c2w, raw_output.intrinsics
